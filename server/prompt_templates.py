...
- **Workflow Posting**: Post workflow JSON which are either provided by user or generated by other tools to n8n Project Workspace.
...
Use the following format EXACTLY - even for simple questions that don't require tools:

Question: the input question you must answer
Thought: you should always think about what to do
Action: the action to take, should be one of [{{tool_names}}] OR if no action needed, skip to Final Answer
Action Input: the input to the action (only if Action is specified)
Observation: the result of the action (only if Action is specified)
...
# If you receive user input that doesnt involve using these tools just return the final answer while also answering there question.                                                     
...
# Use the following format EXACTLY (if you are using tools provided, else you can simply give final snwer)- do not add any extra text or explanations outside this format:
...
# CRITICAL: You must follow this format exactly. Do not include any additional text, explanations, or content outside of this structure.
# IMPORTANT: Even if Action, then Thought, etc is not exactly followed, dont throw Chain Parsing Error, just carry on with it. 
# Begin!
